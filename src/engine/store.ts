...
import type { Character, GamePhase, GameLogEntry, GameState, PotionType } from './types';
...
interface GameActions {
  ...
  logEvent: (entry: Partial<GameLogEntry>) => void;
  ...
  persist(
    (set, get) => ({
      ...
      goToSection: (sectionId: number) => {
        const { character, gameLog } = get();
        if (!character) return;

        const entry: GameLogEntry = {
          timestamp: Date.now(),
          from: character.currentSection,
          to: sectionId,
          event: 'choice',
        };
      ...
      logEvent: (entry: Partial<GameLogEntry>) => {
        const { character, gameLog } = get();
        if (!character) return;
        const full: GameLogEntry = {
          timestamp: Date.now(),
          from: character.currentSection,
          to: entry.to ?? character.currentSection,
